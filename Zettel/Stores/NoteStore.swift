...
class NoteStore: NSObject, ObservableObject, NSFilePresenter {
    ...
    /// Indicates if a changelog is currently being displayed
    @Published var isShowingChangelog: Bool = false
    ...
    // MARK: - Changelog Support
    
    /// Checks if there's a new changelog to display and shows it as the current note
    func checkAndShowChangelog() {
        let changelogManager = ChangelogManager.shared
        changelogManager.checkForNewChangelog()
        
        guard let changelog = changelogManager.pendingChangelog else {
            return
        }
        ...
        self.isShowingChangelog = true
        
        // Clear original filename since this is not a saved note
        originalFilename = nil
        isCurrentNotePersisted = false
    }
    ...
    func dismissChangelog() {
        ...
        // Create a fresh note (same as clearCurrentNote but without saving the changelog)
        var newNote = Note(title: "", content: "")
        newNote.title = newNote.autoGeneratedTitle
        self.currentNote = newNote
    ...
    func clearCurrentNote() {
        // If showing changelog, dismiss it instead of saving
    ...
    private func scheduleAutoSave() {
        ...
        // Don't auto-save changelog notes
        guard !isShowingChangelog else { return }
